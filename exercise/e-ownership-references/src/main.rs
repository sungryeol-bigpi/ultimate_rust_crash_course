...
fn main() {
    ...
    // String reference
    //
    let is_plural = inspect_plural(&arg);
    let plural_disp = if is_plural { "plural" } else { "singular " };
    println!("the word {} is {}", arg, plural_disp);
    ...
    //
    if !is_plural { change_to_plural(&mut arg) }
...
fn inspect_plural(word: &String) -> bool {
    return word.ends_with("s");
}

fn change_to_plural(word: &mut String) {
    word.push_str("s")
}

fn is_banana(word: String) -> bool{
    println!("banana detection on word '{}'", word);
    let last_index = word.len() - 1;
    let before_last = word.chars().nth(last_index - 1);
    println!("word before the last {}", before_last.unwrap());
    return word.starts_with("b") && before_last.unwrap() == 'a';
}
